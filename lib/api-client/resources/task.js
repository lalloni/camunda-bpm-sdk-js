 ...
 * @param {String} [params.id]               The id of the task to create the attachement to.
 * @param {String} [params.data]             The FormData instance containing the form content required by Camunda REST API.
 * @param {String} [params.data.name]        The name of the attachment.
 * @param {String} [params.data.description] The description of the attachment.
 * @param {String} [params.data.type]        The type of the attachment.
 * @param {String} [params.data.url]         The url to the remote content of the attachment.
 * @param {String} [params.data.content]     The content of the attachment.
 */
Task.attachment = function(data, done) {
  done = done || noop;

  if (!data.id) {
    var err = new Error('Task attachment needs a task id.');
    done(err);
    return Q.reject(err);
  }

  if (!data.data) {
    var err = new Error('Task attachment needs the attachment data.');
    done(err);
    return Q.reject(err);
  }
  ...
  return this.http.post(this.path + '/' + data.id + '/attachment/create', {
    data: formData,
    done: done
  });
};
